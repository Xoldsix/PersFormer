...
def define_args():
    ...
    # PersFormer settings
    parser.add_argument('--mod', type=str, default='3D_LaneNet', help='model to train')
...
class Visualizer:
    ...
    def draw_on_img_category(self, img, pred_anchors, gt_anchors, P_g2im, KRT, draw_type='laneline'):
        """
        ...
        for j in range(lane_anchor.shape[0]):
            # draw laneline
            line_pred = {}
            lane_cate = np.argmax(lane_anchor[j, self.anchor_dim-self.num_category:self.anchor_dim])  # 类别
    ...
    def save_result_new(self, dataset, train_or_val, epoch, idx, images, gt, pred, pred_cam_pitch, pred_cam_height,
        ...
        for i in range(idx.shape[0]):
            ...
            H_g2im, P_g2im, H_crop, H_im2ipm, KRT= dataset.transform_mats(idx[i])
            ...
            im_laneline = im.copy()
            fig, img = self.draw_on_img_category(im_laneline, pred_anchors, gt_anchors, P_gt, KRT, 'laneline')
...
def projective_transformation_new(KR, Kt, x_values, y_samples, z_values):
    x_values = np.array(x_values).reshape(-1, 1)
    y_samples = np.array(y_samples).reshape(-1, 1)
    z_values = np.array(z_values).reshape(-1, 1)
    car_lane_new =  np.concatenate((y_samples, -x_values, z_values), 1)
    points = np.dot(car_lane_new, KR.T) + Kt
    # points = np.dot(car_lane, KR.T) + Kt
