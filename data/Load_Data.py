...
class LaneDataset(Dataset):
    ...
    def __init__(self, dataset_base_dir, json_file_path, args, data_aug=False, save_std=False, seg_bev=False):
        ...
        if args.fix_cam:
            ...
            self.P_g2im = projection_g2im(self.cam_pitch, self.cam_height, args.K)   # ===================================================================================================camera
        ...
        if self.use_default_anchor:
            ...
            self.anchor_x_steps = np.linspace(self.x_min, self.x_max, self.anchor_num_before_shear, endpoint=True)  # self.ipm_w // 8
            self.anchor_y_steps = args.anchor_y_steps
            self.num_y_steps = len(self.anchor_y_steps)
            ...
            self.anchor_num = self.anchor_num_before_shear*7  #   self.anchor_num_before_shear * 7
        ...
        if 'openlane' in self.dataset_name:
            ...
            self._label_image_path, \
                ...
                self._im_anchor_origins, \
                self._im_anchor_angles, self._gt_cam_KRT_all = self.init_dataset_3D(dataset_base_dir, json_file_path)
            args.im_anchor_origins = self._im_anchor_origins
            args.im_anchor_angles = self._im_anchor_angles
    ...
    def preprocess_data_from_json_once(self, idx_json_file):
        ...
        if not self.fix_cam:
            ...
            if 'openlane' in self.dataset_name or 'once' in self.dataset_name or 'haomodata' in self.dataset_name:
    ...
    def WIP__getitem__(self, idx):
        ...
        # elif 'apollo' in self.dataset_name: # ======================================================================
            ...
            KRT = self._gt_cam_KRT_all[idx]
            self.K = np.array(KRT[0])
            cam_intrinsics = self.K
        ...
        for i, lane in enumerate(gt_lanes):
            ...
            if self.no_3d:
                x_2d = lane[:, 0]
                y_2d = lane[:, 1]
                H_g2im, P_g2im, H_crop, H_im2ipm, KRT = self.transform_mats_impl(cam_extrinsics, \
                                                    cam_intrinsics, _label_cam_pitch, _label_cam_height, KRT)
                M = H_crop
                ...
                KR = np.dot(K, T_car2cam[: 3, : 3])
                Kt = np.dot(K, T_car2cam[: 3, 3])
                # x_2d, y_2d = projective_transformation(M, lane[:, 0],lane[:, 1], lane[:, 2])
                x_2d, y_2d = projective_transformation_new(KR, Kt, lane[:, 0], lane[:, 1], lane[:, 2])
                gt_laneline_img[i] = np.array([x_2d, y_2d]).T.tolist()
        ...
        # for i in range(gt_anchor.shape[0]):
        # # for i in range(len(gt_lanes)): 
        #     lane_cate = gt_anchor[i, -1]
        #     if lane_cate < 0.5:
        #         continue
        #     xx = gt_anchor[i, :self.num_y_steps] + self.anchor_grid_x[i]
        #     x_2d, y_2d = projective_transformation_new(KR, Kt, xx, self.anchor_y_steps, gt_anchor[i, self.num_y_steps:2*self.num_y_steps])

        #     # x_2d, y_2d = projective_transformation_new(KR, Kt, gt_lanes[i][:,0], gt_lanes[i][:,1], gt_lanes[i][:,2])

        #     x_2d = [int(x) for c, x in enumerate(x_2d)]
        #     y_2d = [int(y) for c, y in enumerate(y_2d)]
        #     for p in range(len(x_2d)-1):
        #         cv2.circle(img, (x_2d[p], y_2d[p]), 1, (255,255,255), 1)
        #         cv2.circle(img, (x_2d[p + 1], y_2d[p + 1]), 1, (255,255,255), 1)
        #         cv2.line(img, (x_2d[p], y_2d[p]), (x_2d[p + 1], y_2d[p + 1]), (0,0,255), 1)   
        # cv2.imwrite(f"/mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/haomodata/gt_img/%s"%(img_path.split("/")[-1]), img)  
    ...
    def init_dataset_3D(self, dataset_base_dir, json_file_path):
        ...
        with open(json_file_path, 'r') as file:
            for line in file:
                ...
                gt_lane_pts = info_dict['laneLines']             # ==================================================================laneLines
                gt_lane_visibility = info_dict['laneLines_visibility']  # ===========================================================laneLines_visibility
                ...
                if 'category' in info_dict:
                    ...
                    gt_laneline_category_all.append(np.ones(len(gt_lane_pts), dtype=np.int32))  # ========================将类别全部设为1
                ...
                if not self.fix_cam:
                    ...
                    gt_cam_K = info_dict["K"]
                    gt_cam_R = info_dict["R"]
                    gt_cam_T = info_dict["T_cam2car"]

                    gt_cam_KRT_all.append((gt_cam_K, gt_cam_R, gt_cam_T))
        ...
        for idx in range(len(gt_laneline_pts_all)):
            ...
            K, R, T_cam2car = gt_cam_KRT_all[idx]
            gt_cam_K = K
            T_car2cam = np.linalg.inv(T_cam2car)
            KR = np.dot(K, T_car2cam[: 3, : 3])
            Kt = np.dot(K, T_car2cam[: 3, 3])
            if not self.fix_cam:
                P_g2im = projection_g2im(gt_cam_pitch, gt_cam_height, gt_cam_K)    # self.K
                H_g2im = homograpthy_g2im(gt_cam_pitch, gt_cam_height, gt_cam_K)
                H_im2g = np.linalg.inv(H_g2im)
            else:
                P_g2im = self.P_g2im
                H_im2g = self.H_im2g
            ...
            for gt_lane in gt_lanes:
                ...
                x_vals, y_vals = projective_transformation_new(KR, Kt, gt_lane[:,0], gt_lane[:,1], gt_lane[:,2])
            ...
            # prune out-of-range points are necessary before transformation   y [0 : 200] 
            gt_lanes = [prune_3d_lane_by_range(gt_lane, 3*self.x_min, 3*self.x_max) for gt_lane in gt_lanes]
            gt_lanes = [lane for lane in gt_lanes if lane.shape[0] > 1]
        ...
        # for idx in range(len(gt_laneline_pts_all)):
        #     assids = gt_laneline_ass_ids[idx]
        #     visibility = gt_laneline_visibility_all[idx]
        #     img_path = label_image_path[idx]
        #     lanes_gt = gt_laneline_pts_all[idx]
        #     K, R, T_cam2car = gt_cam_KRT_all[idx]
        #     gt_cam_K = K
        #     T_car2cam = np.linalg.inv(T_cam2car)
        #     KR = np.dot(K, T_car2cam[: 3, : 3])
        #     Kt = np.dot(K, T_car2cam[: 3, 3])
        #     img = cv2.imread(img_path)
        #     for i in range(len(lanes_gt)):
        #         xx = self.anchor_grid_x[assids[i]] + lanes_gt[i][:,0]
        #         zz = lanes_gt[i][:,1]
        #         yy = self.anchor_y_steps
        #         vis = np.array(visibility[i])
        #         print(vis)
        #         x_2d, y_2d = projective_transformation_new(KR, Kt, xx, yy, zz)
        #         x_2d = [int(x) for c, x in enumerate(x_2d) if vis > 0.5]
        #         y_2d = [int(y) for c, y in enumerate(y_2d) if vis > 0.5]
        #         for p in range(len(x_2d)-1):
        #             cv2.circle(img, (x_2d[p], y_2d[p]), 1, (255,255,255), -1)
        #             cv2.circle(img, (x_2d[p + 1], y_2d[p + 1]), 1, (255,255,255), -1)
        #             cv2.line(img, (x_2d[p], y_2d[p]), (x_2d[p + 1], y_2d[p + 1]), (0,0,255), 3)   
        #     cv2.imwrite(f"/mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/haomodata/gt_img/%s"%(img_path.split("/")[-1]), img)  
        ...
        return label_image_path, gt_laneline_pts_all_org,\
               ...
               anchor_origins, anchor_angles, gt_cam_KRT_all
    ...
    def transform_mats(self, idx, dataneme="haomodata"):
        """
        ...
        elif dataneme == "haomodata":
            idx_json_file = self[idx][0]
            with open(idx_json_file, 'r') as file:
                file_lines = file.read()
                info_dict = json.loads(file_lines)
                cam_extrinsics = np.array(info_dict['extrinsic'])
                R_vg = np.array([[0, 1, 0],
                                [-1, 0, 0],
                                [0, 0, 1]], dtype=float)
                ...
                K = info_dict['K']
                R = info_dict['R']
                T_cam2car = info_dict["T_cam2car"]
                cam_intrinsics = np.array(cam_intrinsics)
            _label_cam_height = gt_cam_height
            _label_cam_pitch = gt_cam_pitch
            return self.transform_mats_impl(cam_extrinsics, cam_intrinsics, _label_cam_pitch, _label_cam_height, (K, R, T_cam2car))
        
        elif dataneme == "apollo":
            idx_json_file = self[idx][0]
            with open(idx_json_file, 'r') as file:
                file_lines = file.read()
                info_dict = json.loads(file_lines)
    ...
    def transform_mats_impl(self, cam_extrinsics, cam_intrinsics, cam_pitch, cam_height, KRT):
        if not self.fix_cam:
            if 'openlane' in self.dataset_name:
            ...
            H_im2ipm = np.linalg.inv(np.matmul(self.H_crop, np.matmul(H_g2im, self.H_ipm2g)))
            return H_g2im, P_g2im, self.H_crop, H_im2ipm, KRT
        else:
            return self.H_g2im, self.P_g2im, self.H_crop, self.H_im2ipm, KRT
...
def get_loader(transformed_dataset, args):
    ...
    if args.dist:
        ...
        if 'standard' in args.dataset_name or 'rare_subset' in args.dataset_name or 'illus_chg' in args.dataset_name or 'haomodata' in args.dataset_name:
