...
class PersFormer(nn.Module):
    def __init__(self, args):
        ...
        # 2d lane detector
        self.shared_encoder = Lane2D.FrontViewPathway(args.feature_channels, args.num_proj)  # (128, 4)  
        stride = 2
        self.laneatt_head = Lane2D.LaneATTHead(stride * pow(2, args.num_proj - 1),
                                               ...
                                               img_w=args.resize_w,                             # 480
                                               img_h=args.resize_h,                             # 360
                                               S=args.S,                                        # 72
                                               anchor_feat_channels=args.anchor_feat_channels,  # 64
                                               num_category=args.num_category)                  # 1
        ...
        self.bev_head = BEVHead(args, channels=args.feature_channels)    # bev_head优化得到完整分辨率bev特征
        # 3d lane detector
    ...
    def forward(self, input, _M_inv = None):
        out_featList = self.encoder(input)  # ResNext101
