...
class FrontViewPathway(nn.Module):
    def __init__(self, input_channels, num_proj, init_weights=True):
        ...
        output_channels = input_channels  # 128
        for i in range(num_proj - 1):
            if i < num_proj - 2:
                output_channels *= 2
            layers = []
            layers += [nn.MaxPool2d(kernel_size=2, stride=2)]                                           # 128
            conv2d = nn.Conv2d(input_channels, output_channels, kernel_size=3, padding=1)
            layers += [conv2d, nn.BatchNorm2d(output_channels), nn.ReLU(inplace=True)]                  # 256         
            conv2d_add = nn.Conv2d(output_channels, output_channels, kernel_size=3, padding=1)
            layers += [conv2d_add, nn.BatchNorm2d(output_channels), nn.ReLU(inplace=True)]              # 256
            layers += [conv2d_add, nn.BatchNorm2d(output_channels), nn.ReLU(inplace=True)]              # 256
            self.layers.append(nn.Sequential(*layers))
            input_channels = output_channels                                                            #二倍降采样
...
class LaneATTHead(nn.Module):
    def __init__(self, stride, input_channels, im_anchor_origins, im_anchor_angles,
        ...
        self.conv1 = nn.Conv2d(input_channels, self.anchor_feat_channels, kernel_size=1)   # 128 * 4
