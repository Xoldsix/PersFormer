...
class Runner:
    def __init__(self, args):
        ...
        self.val_gt_file = ops.join(args.data_dir, 'test.json')    # {"raw_file": "images/03/0000096.jpg", "cam_height": 1.7860000133514404, "cam_pitch": 0.06047958880662918, 
                                                                      #"centerLines": [[[24.097721099853516, 91.01872813788557, 0.22746204719246332]]],
                                                                      # "laneLines":[[[[-1.7175495624542236, 4.635233731980652, -0.004180214260614296]]],
                                                                      # "centerLines_visibility": [[0~1]], laneLines_visibility:[[0~1]]}
        ...
        if 'apollo' in args.dataset_name or 'haomodata' in args.dataset_name:
        ...
        if 'openlane' in args.dataset_name:
            self.evaluator = eval_3D_lane.LaneEval(args) # 3D车道线评估
    ...
    def train(self):
        ...
        for epoch in range(args.start_epoch, args.nepochs):
            ...
            # Specify operation modules 指定操作模块
            if args.model_name == "PersFormer":
                model.train()
            elif args.model_name == "GenLaneNet":
                model2.train()
            ...
            for i, (json_files, input, seg_maps, gt, gt_laneline_img, idx, gt_hcam, gt_pitch, gt_intrinsic, gt_extrinsic, aug_mat, seg_name, seg_bev_map) in tqdm(enumerate(train_loader)):
                ...
                # Put inputs on gpu if possible 如果可能的话，将输入放在gpu上
                if not args.no_cuda:
                ...
                # update transformation based on gt extrinsic/intrinsic基于外/内的更新转换gt
                if args.dataset_name == 'apollo' or args.dataset_name == 'haomodata':
                ...
                if loss.data > args.loss_threshold:
                    ...
                    loss.fill_(0.0)
                # Clip gradients (usefull for instabilities or mistakes in ground truth) 剪辑梯度(有用的不稳定或错误的地面真相)
                ...
                # reduce loss from all gpu, then update losses 减少所有gpu的损耗，然后更新loss
                if args.model_name == "PersFormer":
    ...
    def validate(self, model, model2=None, epoch=3, vis=False):
        args = self.args
        ...
        with torch.no_grad():
            for i, (json_files, input, seg_maps, gt, gt_laneline_img, idx, gt_hcam, gt_pitch, gt_intrinsic, gt_extrinsic, seg_name, seg_bev_map) in tqdm(enumerate(loader)):
                ...
                if args.dataset_name == 'apollo' or args.dataset_name == 'haomodata':
                    ...
                    M_inv = unit_update_projection_extrinsic(args, gt_extrinsic, gt_intrinsic)   # M_inv:ipm to im
                ...
                if ((i + 1) % args.save_freq == 0 or args.evaluate) and args.model_name == "PersFormer": # ===================================================
                    ...
                    for j in range(num_el):
                        ...
                        if args.dataset_name == 'openlane':
                            ...
                            img_name =  os.path.basename(json_file).replace('.txt', '.jpg')
                            img_name_all.append(img_name)
                ...
                for j in range(num_el):
                    ...
                    if 'once' in args.dataset_name:
                        if 'training' in json_file:
                            ...
                            img_path = json_file.replace('test', 'data').replace('.json', '.jpg')
                        json_line["file_path"] = img_path
                    elif 'apollo' in args.dataset_name or 'haomodata' in args.dataset_name : # ===================================================================
                    ...
                    if args.evaluate and args.model_name == "PersFormer":
                        img_path = json_line["file_path"]
                        if args.dataset_name == 'openlane' or args.dataset_name == 'haomodata':
            ...
            if 'openlane' in args.dataset_name or 'apollo' in args.dataset_name or 'haomodata' in args.dataset_name :
    ...
    def eval(self):
        ...
        if os.path.isfile(best_file_name):
            ...
            if (args.proc_id == 0) and (args.model_name == "PersFormer"):
                ...
                # pretrained_dict = {k: v for k, v in pretrained_dict.items() if (k in model_dict and 'lane_out.dim_rt.3' not in k)}
    ...
    def _get_train_dataset(self):
        ...
        if 'openlane' in args.dataset_name:
            ...
            train_dataset.normalize_lane_label() # 'centerLines' or 'laneLines'   ------>train.json
