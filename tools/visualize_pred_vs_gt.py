...
class lane_visualizer(object):
    ...
    def visualize_lanes(self, car_lanes, gt_lanes, pred_lanes, raw_file, gt_cam_height, gt_cam_pitch, K, T_cam2car, ax1, ax2, ax3):
        self.K = K
        # P_g2im = projection_g2im(gt_cam_pitch, gt_cam_height, self.Ks)
        T_car2cam = np.linalg.inv(T_cam2car)
        KR = np.dot(K, T_car2cam[: 3, : 3])
        Kt = np.dot(K, T_car2cam[: 3, 3])
        Kt2 = Kt.reshape((3,1))
        P_g2im = np.concatenate([KR.T, Kt2], 1)
        ...
        # print(ops.join(self.dataset_dir, raw_file))
        ...
        # img_lanes = []
        # for lane_img in car_lanes:
        #     points = np.dot(lane_img, KR.T) + Kt
        #     img_points, zs = points[:, 0: 2], points[:, 2]
        #     img_points = points[:, 0: 2] / zs[:, None]
        #     points_new = []
        #     for i, n in enumerate(img_points):
        #         points_new.append([n[0],n[1],i])
        #     #  These limitations will filter the far points, which cuts the `gt` lines.
        #     valid_mask = (img_points[:, 0] >= 0) & (img_points[:, 0] < 1920) \
        #                 & (img_points[:, 1] >= 0) & (img_points[:, 1] < 1080) \
        #                 & (zs >= 0)

        #     (row_idxs, *_) = np.where(valid_mask)
        #     img_points = img_points[row_idxs, :]
        #     img_lanes.append(img_points)
        # for cam_points_new in img_lanes:
        #     if len(cam_points_new) <= 1:
        #         continue
        #     color = (0, 0, 255)  
        #     for i in range(len(cam_points_new) - 1):

        #         x, y = cam_points_new[i][0], cam_points_new[i][1]
        #         x2, y2 = cam_points_new[i + 1][0], cam_points_new[i + 1][1]
        #         cv2.circle(img2, (int(x), int(y)), 1, color, -1)
        #         cv2.circle(img2, (int(x2), int(y2)), 1, color, -1)
        #         cv2.line(img2, (int(x), int(y)), (int(x2), int(y2)), color, 1)
        # cv2.imwrite(f"/mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/haomodata/img/%s"%(raw_file.replace("/", "_")), img2)
        ...
        for i in range(cnt_gt):
            ...
            lanes.append(gt_lanes[i])
            gt_lanes[i] = np.vstack([x_values, z_values]).T
            insert_lanes.append(np.concatenate((np.array(gt_lanes[i][:,0]).reshape(-1, 1), np.array(self.y_samples).reshape(-1, 1), np.array(gt_lanes[i][:,1]).reshape(-1, 1)), 1))
            gt_visibility_mat[i, :] = np.logical_and(x_values >= self.x_min,
                                                       np.logical_and(x_values <= self.x_max,     #　　x[-10, 10]
        ...
        # self.get_mask(insert_lanes, raw_file.replace("/", "_"))
        ...
        for i in range(cnt_gt):
            ...
            # x_2d, y_2d = projective_transformation(P_gt, -self.y_samples, x_values, z_values)
            lanes[i] = np.array(lanes[i])
            # x_2d, y_2d = projective_transformation_new(KR, Kt, car_lanes[i], lanes[i][:,0], lanes[i][:,1], lanes[i][:,2])
            x_2d, y_2d = projective_transformation_new(KR, Kt, car_lanes[i], x_values, self.y_samples, z_values)   #  ========================================
            ...
            for k in range(1, x_2d.shape[0]):   
                ...
                if gt_visibility_mat[i, k - 1] and gt_visibility_mat[i, k]:
                    img = cv2.line(img2, (x_2d[k - 1], y_2d[k - 1]), (x_2d[k], y_2d[k]), color[-1::-1], 10)
        ...
        for i in range(cnt_pred):
            ...
            for k in range(1, x_2d.shape[0]):
                ...
                if pred_visibility_mat[i, k - 1] and pred_visibility_mat[i, k]:
                    img = cv2.line(img, (x_2d[k - 1], y_2d[k - 1]), (x_2d[k], y_2d[k]), color[-1::-1], 6)
    ...
    def get_mask(self, center_points_list, idx):
        ...
        # if lb_points_list is not None:
        #     for lb_points in lb_points_list:
        #         ys = lb_points[:, 0]
        #         xs = lb_points[:, 1] * -1
        #         plt.plot(xs, ys, color=(255, 255, 255))
        # if rb_points_list is not None:
        #     for rb_points in rb_points_list:
        #         ys = rb_points[:, 0]
        #         xs = rb_points[:, 1] * -1
        #         plt.plot(xs, ys, color=(255, 255, 255))
        plt.grid(visible=True)
        plt.savefig(f'//mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/dataset/apollo/data_splits/standard/insert_gt/%s'%(idx))
        plt.close()
...
if __name__ == '__main__':
    ...
    args.dataset_name = 'standard'
    # args.dataset_dir = '/mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/dataset/apollo/Apollo_Sim_3D_Lane_Release'  
    args.dataset_dir = '/mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/haomodata'  
    # model name: 'Gen_LaneNet_ext' / '3D_LaneNet'
    model_name = '3D_LaneNet'
    ...
    pred_file = "/mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/haomodata/test.json"
    # gt_file = '../data_splits/' + args.dataset_name + '/test.json'
    gt_file = "/mnt/ve_perception/likaiying/opendataset/sandbox/PersFormer_3DLane_main/haomodata/test.json"
    ...
    for i, pred in enumerate(json_pred):
        ...
        K = gt['K']
        R = gt['R']
        T_cam2car = gt["T_cam2car"]
        car_lanes = gt["car_points"]
        ...
        # draw lanes
        vs.get_mask(gt_lanelines, raw_file.replace("/", "_"))
        vs.visualize_lanes(car_lanes, gt_lanelines, pred_lanelines, raw_file, gt_cam_height, gt_cam_pitch, K, T_cam2car, ax1, ax2, ax3)
        vs.visualize_lanes(car_lanes, gt_centerlines, pred_centerlines, raw_file, gt_cam_height, gt_cam_pitch, K, T_cam2car, ax4, ax5, ax6)
        ax1.set_xticks([])
        ax1.set_yticks([])
        ax2.set_xticks([])
        ax2.set_yticks([])
